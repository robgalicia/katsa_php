...
DROP FUNCTION IF EXISTS fn_importeletra $$

create function fn_importeletra(lnImporte decimal(18,2)) returns varchar(250)
begin
	...
	SET lnEntero = FLOOR(lnImporte);
	SET lnFraccion = (lnImporte - lnEntero) * 100;
	...
	SET lcRetorno = CONCAT(lcRetorno,' PESOS ',lpad(lnFraccion,2,'0'),'/100 M.N.');
