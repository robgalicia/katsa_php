...
DROP PROCEDURE IF EXISTS sp_clientedomiciliofiscal_ups $$

CREATE PROCEDURE sp_clientedomiciliofiscal_ups
(IN pidclientedomiciliofiscal         smallint,
IN pidcliente            int,
...
begin
	if pidclientedomiciliofiscal = 0 then	

		insert into clientedomiciliofiscal(idcliente,nombre,nombrecomercial,calle,numexterior,numinterior,colonia,ciudad,idmunicipio,idestado,
					codigopostal,rfc,personacontacto,telefonocontacto,correoelectronico,cveusocfdi,porcentajeiva,quien,cuando)
		values(pidcliente,pnombre,pnombrecomercial,pcalle,pnumexterior,pnuminterior,pcolonia,pciudad,pidmunicipio,pidestado,
		...
		update clientedomiciliofiscal SET
		...
		SET last_id = pidclientedomiciliofiscal;
	end if;
